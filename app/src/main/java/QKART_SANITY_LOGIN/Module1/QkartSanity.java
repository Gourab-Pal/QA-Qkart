...
public class QkartSanity {
    ...
    public static void takeScreenshot(WebDriver driver, String screenshotType, String description) {
        ...
        try {
            ...
            String ssTimestamp = String.valueOf(java.time.LocalDateTime.now());
            String ssFilename = String.format("screenshot_%s_%s_%s.png", ssTimestamp,
                    screenshotType, description);
            TakesScreenshot ts = (TakesScreenshot) driver;
            File srcFile = ts.getScreenshotAs(OutputType.FILE);
            File destFile = new File("screenshots/" + ssFilename);
    ...
    public static Boolean TestCase01(RemoteWebDriver driver) throws InterruptedException {
        ...
        if (!status) {
            ...
            logStatus("End TestCase", "Test Case 1: Verify user Registration : ",
                    status ? "PASS" : "FAIL");
            takeScreenshot(driver, "Fail", "TestCase01");
    ...
    public static Boolean TestCase02(RemoteWebDriver driver) throws InterruptedException {
        ...
        if (!status) {
            logStatus("End TestCase", "Test Case 2: Verify user Registration : ",
                    status ? "PASS" : "FAIL");
            takeScreenshot(driver, "Fail", "TestCase02");
            return false;
    ...
    public static Boolean TestCase03(RemoteWebDriver driver) throws InterruptedException {
        ...
        if (!status) {
            logStatus("TestCase 3", "Test Case Failure. Unable to search for given product",
                    "FAIL");
            takeScreenshot(driver, "Fail", "TestCase03");
            return false;
        }
            ...
            if (result.verifySizeChartExists()) {
                ...
                if (result.openSizechart()) {
                    ...
                    if (result.validateSizeChartContents(expectedTableHeaders, expectedTableBody,
                        ...
                        takeScreenshot(driver, "Fail", "TestCase04");
                    }
    ...
    public static Boolean TestCase05(RemoteWebDriver driver) throws InterruptedException {
        ...
        if (!status) {
            ...
            takeScreenshot(driver, "Fail", "TestCase05");
        }
    ...
    public static Boolean TestCase06(RemoteWebDriver driver) throws InterruptedException {
        ...
        if (!status) {
            logStatus("TestCase 5", "Test Case Failure. Happy Flow Test Failed", "FAIL");
            takeScreenshot(driver, "Fail", "TestCase06");
        }
        lastGeneratedUserName = registration.lastGeneratedUsername;
    ...
    public static Boolean TestCase07(RemoteWebDriver driver) throws InterruptedException {
        ...
        if (!status) {
            ...
            logStatus("End TestCase",
                    "Test Case 7: Verify that insufficient balance error is thrown when the wallet balance is not enough: ",
                    status ? "PASS" : "FAIL");
            takeScreenshot(driver, "Fail", "TestCase07");
            return false;
        }
        lastGeneratedUserName = registration.lastGeneratedUsername;
    ...
    public static Boolean TestCase08(RemoteWebDriver driver) throws InterruptedException {
        ...
        logStatus("Start TestCase", "Test Case 8: Verify multitab scenerio", "DONE");
        ...
        status = registration.registerUser("testUser", "abc@123", true);
        if (!status) {
            logStatus("TestCase 8", "Test Case Failure. Multitab scenerio user creation Failed",
                    "FAIL");
            takeScreenshot(driver, "Fail", "TestCase08");
        }
        ...
        if (!status) {
            logStatus("Step Failure", "User Perform Login Failed", status ? "PASS" : "FAIL");
            logStatus("End TestCase", "Test Case 5: Multitab scenerio Failed : ",
                    status ? "PASS" : "FAIL");
            takeScreenshot(driver, "Fail", "TestCase08");
        }

        Home home = new Home(driver);
        home.searchForProduct("YONEX Smash Badminton Racquet");
        ...
        home.navigateToHome();
        String productNameNewTab = driver
                .findElement(By.xpath("//div[@class='MuiBox-root css-1gjj37g']/div[1]")).getText();

        status = productNameNewTab.equals("YONEX Smash Badminton Racquet");

        if (!status) {
            takeScreenshot(driver, "Fail", "TestCase08");
        }

        logStatus("End TestCase", "Test Case 8: Verify multitab has correct product: ",
                status ? "PASS" : "FAIL");
        driver.close();
    ...
    public static Boolean TestCase09(RemoteWebDriver driver) throws InterruptedException {
        ...
        logStatus("Start TestCase", "Test Case 9: Privacy policy and terms usage", "DONE");
        Boolean status = false;
        Home home = new Home(driver);
        home.navigateToHome();
        String mainTabUrl = driver.getCurrentUrl();
        driver.findElement(By.linkText("Privacy policy")).click();
        status = mainTabUrl.equals(driver.getCurrentUrl());
        if (!status) {
            takeScreenshot(driver, "Fail", "TestCase09");
        }
        logStatus("Step TestCase", "Test Case 9: Current tab url does not change: ",
                status ? "PASS" : "FAIL");
        ...
        status = privacyElement.getText().equals("Privacy Policy");

        if (!status) {
            takeScreenshot(driver, "Fail", "TestCase09");
        }
        logStatus("Step TestCase", "Test Case 9: Privacy policy content is available: ",
                status ? "PASS" : "FAIL");
        ...
        WebElement termsElement =
                wait.until(ExpectedConditions.presenceOfElementLocated(By.tagName("h2")));
        status = termsElement.getText().equals("Terms of Service");

        if (!status) {
            takeScreenshot(driver, "Fail", "TestCase09");
        }
        logStatus("Step TestCase", "Test Case 9: Terms of Service content is available: ",
                status ? "PASS" : "FAIL");
    ...
    public static Boolean TestCase10(RemoteWebDriver driver) throws InterruptedException {
        ...
        logStatus("Start TestCase", "Test Case 10: Contact us card", "DONE");
        Boolean status = false;
        // TODO: CRIO_TASK_MODULE_SYNCHRONISATION -
        Home home = new Home(driver);
        home.navigateToHome();
        ...
        WebDriverWait wait = new WebDriverWait(driver, 10);
        WebElement contactcard = wait.until(ExpectedConditions
                .visibilityOfElementLocated(By.xpath("//div[@class='card-block']")));
        contactcard.findElement(By.xpath(".//input[@placeholder='Name']")).sendKeys("crio user");
        contactcard.findElement(By.xpath(".//input[@placeholder='Email']"))
                .sendKeys("criouser@gmail.com");
        contactcard.findElement(By.xpath(".//input[@placeholder='Message']"))
        ...
        if (!status) {
            takeScreenshot(driver, "Fail", "TestCase10");
        }
        logStatus("Step TestCase", "Test Case 10: Contact us card disappered: ",
                status ? "PASS" : "FAIL");
    ...
    public static Boolean TestCase11(RemoteWebDriver driver) throws InterruptedException {
        ...
        logStatus("Start TestCase", "Test Case 11: Verify advertisement", "DONE");
        Boolean status = false;
        // TODO: CRIO_TASK_MODULE_SYNCHRONISATION -
        // Go to the Register page
        Register registration = new Register(driver);
        ...
        status = registration.registerUser("testUser", "abc@123", true);
        if (!status) {
            logStatus("TestCase 11", "Test Case Failure. Advertisement Test Failed", "FAIL");
        ...
        if (!status) {
            logStatus("Step Failure", "User Perform Login Failed", status ? "PASS" : "FAIL");
            logStatus("End TestCase", "Test Case 11: Advertisement Test Failed : ",
                    status ? "PASS" : "FAIL");
            takeScreenshot(driver, "Fail", "TestCase11");
        }
        ...
        if (!status) {
            logStatus("Step Failure", "Thanks page", status ? "PASS" : "FAIL");
            takeScreenshot(driver, "Fail", "TestCase11");
        }

        WebElement parentContainer = wait.until(ExpectedConditions.presenceOfElementLocated(By
                .xpath("//div[@class='MuiGrid-root MuiGrid-container MuiGrid-spacing-xs-4 css-92t6i8']")));
        List<WebElement> qkartProd = parentContainer.findElements(By.xpath("./iframe"));
        wait.until(ExpectedConditions
                .frameToBeAvailableAndSwitchToIt(By.xpath("//iframe[@title='scroll']")));
        status = (qkartProd.size() + 1) == 3;
        if (!status) {
            logStatus("Step Failure", "Total three advertise", status ? "PASS" : "FAIL");
            takeScreenshot(driver, "Fail", "TestCase11");
        }

        driver.switchTo().parentFrame();

        int n = qkartProd.size();
        while (n > 0) {
            String xpathFrame = "(//iframe[@class='iframe'])[" + n + "]";
            wait.until(ExpectedConditions.frameToBeAvailableAndSwitchToIt(By.xpath(xpathFrame)));
            driver.findElement(By.xpath("//button[text()='Buy Now']")).click();
            driver.switchTo().parentFrame();
            status = wait.until(ExpectedConditions.urlContains("/checkout"));
            if (!status) {
                logStatus("Step Failure", "Qkart product button clickable", status ? "PASS" : "FAIL");
                takeScreenshot(driver, "Fail", "TestCase11");
            }
            logStatus("Step Success", "Qkart product button clickable", status ? "PASS" : "FAIL");
